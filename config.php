...
function getCompanyFilePath($sub = 'kuitit')
{
    global $COMPANYDIR;
    $vatid = set_safe($_SESSION['vatid']);
    $company_path = $COMPANYDIR .  $vatid.'/'.$sub.'/';


    if (!file_exists($company_path))
    {
    ...
    if ( file_exists($company_path))
    {
        error_log("Created company path: ".$company_path);
        return $company_path;
    }

    error_log("Unable to create path: ".$company_path );
    return "";
